...
const tempQuestions = JSON.parse(fs.readFileSync(path.join(__dirname, 'temp_unit389_questions.json'), 'utf8'));

let content = fs.readFileSync(questionsFile, 'utf8');

// 1. Generate the Quiz Arrays
let quizArraysText = '';
const startId = 2321; // Last ID was 2320
let currentId = startId;

tempQuestions.forEach((quiz, index) => {
    quizArraysText += `const unit389Quiz${index + 1}: Question[] = [\n`;
...
const insertionStr = 'if (unitId === 388) {';
const insertionPoint = content.indexOf(insertionStr);
if (insertionPoint === -1) {
    console.error('Could not find unit 388 mapping');
    process.exit(1);
}

// Find the end of the unit 388 block
let braceCount = 0;
let endIdx = -1;
...
console.log('Successfully inducted Unit 389 quizzes.');
